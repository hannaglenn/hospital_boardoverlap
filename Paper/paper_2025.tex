...
\end{abstract}
  ...
  % These trends show that not all hospitals serve as a suitable comparison group for those that gain a connection within the same HRR. In the top panel, which plots the average number of services over time, hospitals that remain unconnected or always connected in a different HRR follow a different trajectory than hospitals that become connected. A similar pattern appears for discharges: hospitals that gain overlap track a comparable path before the connection, then diverge, while hospitals that never gain overlap exhibit a distinct trend of declining Medicare and Medicaid discharges. Finally, total operating expenses reveal entirely different slopes for hospitals that never gain overlap compared to those that do, whether in the same or a different market. For this reason, when estimating the effect of gaining overlap in the same market, I compare these hospitals only to those that become connected in a different market. This approach allows me to isolate the effect of connecting with a competitor from the broader effect of becoming connected.
    ...
    where $Y_{it}$ captures system integration, separated by whether the system has presence within the same market or only in different markets. $D_{i,t+k}$ marks event time $k$ relative to the year of gaining or losing overlap (same HRR or different HRR), and $\gamma_i$ and $\delta_t$ are hospital and year fixed effects. I estimate separate sets of $\beta_k$ for same-HRR and different-HRR overlap events. These estimates should not be taken as the causal effect of overlap events on system integration, they are meant to provide suggestive evidence that there is a relationship. 

Figure \ref{fig:system} presents estimates when the outcome is an indicator for whether a hospital belongs to a system, capturing both same- and cross-market consolidation. The left panel uses overlap events that occur with hospitals in the same HRR, while the right panel leverages overlap events with hospitals in different HRRs. In the left panel, neither gaining nor losing overlap with a hospital in the same HRR is associated with a statistically significant change in the likelihood of system membership, as the confidence intervals comfortably span zero throughout the post-period. In the right panel, however, losing overlap with a hospital in a different HRR is associated with a modest positive trend in system membership following the overlap event, though estimates remain imprecise. Hospitals that gain overlap with a different-HRR hospital show no meaningful change in consolidation behavior. Importantly, pre-trends are flat across all four series, supporting the parallel trends assumption. Overall, these results suggest that leadership overlap with out-of-market hospitals may serve as a partial substitute for formal system integration, consistent with the hypothesis that cross-market informal affiliations reduce the need to consolidate.
...
Under these assumptions, the estimated event-time coefficients capture the dynamic effect of gaining same-HRR overlap relative to not-yet-treated hospitals — those that will eventually gain same-HRR overlap but have not yet done so. This effect is conditional on eventual connectivity and should be interpreted as the impact of local overlap among an otherwise similar set of hospitals.
    ...
    Figure \ref{fig:qualityoverall} presents estimates for two overall measures of patient-perceived quality: overall star rating and an aggregated summary star rating. In both panels, pre-event coefficients are centered near zero, supporting the parallel trends assumption. Following the gain in leadership overlap with a hospital in the same HRR, both measures show a positive trend, with point estimates drifting upward and becoming more pronounced at event times 3 and 4, though only the summary star rating is statistically significant in these periods. The pattern suggests that overlap improves quality, but is consistent with the long-run nature of organizational change.
