...
\end{abstract}
  ...
  These trends show that not all hospitals serve as a suitable comparison group for those that gain a connection within the same HRR. In the top panel, which plots the average number of services over time, hospitals that remain unconnected or always connected in a different HRR follow a different trajectory than hospitals that become connected. A similar pattern appears for discharges: hospitals that gain overlap track a comparable path before the connection, then diverge, while hospitals that never gain overlap exhibit a distinct trend of declining Medicare and Medicaid discharges. Finally, total operating expenses reveal entirely different slopes for hospitals that never gain overlap compared to those that do, whether in the same or a different market. For this reason, when estimating the effect of gaining overlap in the same market, I compare these hospitals only to those that become connected in a different market. This approach allows me to isolate the effect of connecting with a competitor from the broader effect of becoming connected.
    ...
    \subsection{Mergers/Acquisitions}
     ...
     The underlying nature of overlapping leadership with regards to other types of hospital affiliations is an open empirical question. Theoretically, informal affiliations could act as either a substitute for formal mergers or a complement to formal mergers. If they are substitutes, hospitals leverage informal affiliation to achieve some benefits of consolidation, such as strategic alignment, resource coordination, and cost efficiencies, without incurring the regulatory scrutiny and transaction costs associated with a merger. Alternatively, informal affiliations may complement formal mergers either by establishing a relationship that eventually leads to a merger, or by occurring as a result of intentions to merge in the future.

    I first investigate the correlation between the timing of gaining or losing leadership overlap and formally integrating with a system using the following stacked event study specification: 
    
    \begin{equation}\label{eq:consolidation}
    Y_{it} = \alpha + \sum_{k \neq -1} \beta_k \cdot D_{i,t+k} + \gamma_i + \delta_t + \varepsilon_{it},
    \end{equation}

    where $Y_{it}$ captures system integration, separated by whether the system has presence within the same market or only in different markets. $D_{i,t+k}$ marks event time $k$ relative to the year of gaining or losing overlap (same HRR or different HRR), and $\gamma_i$ and $\delta_t$ are hospital and year fixed effects. I estimate separate sets of $\beta_k$ for same-HRR and different-HRR overlap events. These estimates should not be taken as the causal effect of overlap events on system integration; they are simply an insight into the timing of such events. 

    Figure \ref{fig:any_formal_sameHRR} presents these estimates when the outcome is limited to system integration within the same HRR market. The left panel uses overlap events that occur with hospitals in the same market, while the right panel leverages overlap events with hospitals in different markets. Losing overlap with a hospital in the same HRR is associated with higher likelihood of entering a system in the same HRR. This is partly mechanical, but provides insight that overlap could pre-date mergers in the same HRR. Similarly, Figure \ref{fig:any_formal_diffHRR} shows the correlation between overlap events and system integration in a different HRR. When hospitals lose overlap with a hospital located in a different HRR, they are also less likely to integrate with a system in a different HRR. Similar patterns are shown in the other overlap events, though not statistically significant. Overall, these graphs point to mergers and overlap acting together, where overlap events coincide with changes in the probability of integrating into a system. 
    ...
    This estimation provides some insight into whether there is correlation in the timing of these events, but gives little insight into the true nature of how overlap and mergers are related. Thus, I leverage changes in the ``price" of merging, and see how this affects informal affiliation. Formal mergers and acquisitions are nationally regulated, and the extent to which they are regulated depends on characteristics of the national government such as political party in office. However, informal affiliations are not nationally regulated, and thus not directly affected by changes in merger regulation ``aggressiveness". If the two are complements, more aggressive regulation on mergers should lead to less informal affiliations as well. If the two are substitutes, more aggressive regulations on mergers should lead to increases in informal affiliations. 

    I measure the aggressiveness of the national regulation on mergers using historical data on the proportion of proposed mergers that have actions taken against them by the Federal Trade Commission or the Department of Justice.\footnote{\href{https://www.ftc.gov/policy/reports/annual-competition-reports}{https://www.ftc.gov/policy/reports/annual-competition-reports}} I then estimate the following equations: 
