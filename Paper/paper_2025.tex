...
\begin{abstract}
\small{I examine non-merger affiliations among U.S. nonprofit hospitals by constructing a measure of leadership overlap: shared directors or executives across otherwise unaffiliated hospitals. I document that 10\% of hospitals share a board member with a competitor in the same Hospital Referral Region (HRR) and that 25\% of HRRs contain at least one such affiliation. Leveraging changes in regulation of formal mergers, I show that mergers and overlap within the same HRR are unrelated, but mergers and overlap in different HRRs are substitutes. I then estimate the effect of gaining or losing local overlap on hospital behavior by comparing hospitals that experience an overlap event in the same market to those that experience overlap in a different market, isolating the incremental effect of local connections relative to non-local affiliations. I find little evidence that overlap is anti-competitive. Hospitals that gain overlap in the same HRR exhibit higher quality, seemingly through better care coordination, and offer more services. These results are consistent with overlap representing broader informal affiliations such as joint ventures. }
\end{abstract}
  ...
  % These trends show that not all hospitals serve as a suitable comparison group for those that gain a connection within the same HRR. In the top panel, which plots the average number of services over time, hospitals that remain unconnected or always connected in a different HRR follow a different trajectory than hospitals that become connected. A similar pattern appears for discharges: hospitals that gain overlap track a comparable path before the connection, then diverge, while hospitals that never gain overlap exhibit a distinct trend of declining Medicare and Medicaid discharges. Finally, total operating expenses reveal entirely different slopes for hospitals that never gain overlap compared to those that do, whether in the same or a different market. For this reason, when estimating the effect of gaining overlap in the same market, I compare these hospitals only to those that become connected in a different market. This approach allows me to isolate the effect of connecting with a competitor from the broader effect of becoming connected.
    ...
    \subsection{Mergers/Acquisitions}
...
\end{table}
    ...
    I present the results of this estimation in Table \ref{tab:subs_comp}. Columns (1) and (2) show that the merger aggressiveness does in fact decrease hospital propensity to be in a system, indicating this is a good measure of the cost of merging. Column (3) shows that both current and lagged merger cost is largely unrelated to hospital likelihood of having leadership overlap with hospitals in the same HRR, as these estimates are small in magnitude and not statistically significant. Column (4) shows that both current and lagged merger costs lead to more occurrences of informal affiliation with hospitals in different HRRs, since these estimates are positive and statistically significant. A potential explanation for this is that the pool of hospitals in the same HRR is limited, so merger regulations do not affect relationships within that pool as much as they do for all options of hospitals outside the market. 

    \import{Objects}{subs_comp.tex}

    Taken together, we see that overlap events do seem to be correlated with the timing of system integration events in a way that could indicate a complements relationship. However, this relationship could be mechanical as hospitals in the same system are automatically ineligible to be informally affiliated. We also see that merger regulation leads to an increase in informal affiliation, only in cases outside of the same market, indicating a substitutes relationship. \textcolor{red}{finish this with a more clear conclusion??}
    ...
    \begin{subfigure}[b]{0.47\textwidth}
        ...
        \includegraphics[width=\textwidth]{Objects/overall_rating.pdf}
        \label{fig:overall_rating}
    \end{subfigure}
    \begin{subfigure}[b]{0.47\textwidth}
        \caption{Summary Star Rating}
        \includegraphics[width=\textwidth]{Objects/summary_rating.pdf}
    ...
    \begin{subfigure}[b]{0.47\textwidth}
        ...
        \includegraphics[width=\textwidth]{Objects/care_transition.pdf}
        \label{fig:care_transition}
    \end{subfigure}
    \begin{subfigure}[b]{0.47\textwidth}
        \caption{Doctor Communication}
        \includegraphics[width=\textwidth]{Objects/doc_communication.pdf}
