    ...
    \begin{columns}
        ...
        \column{0.5\textwidth}
            \begin{wideitemize}
                ...
                \item Now more concern over lack of competition
            \end{wideitemize}
    \end{columns}
\end{frame}
...
\begin{tikzpicture}[
  ...
  \draw[underarr]
    ...
    ($(comp.south) + (0,-0.1cm)$) -- (comp);
      ...
      % --- New literature box above the middle of the cons<->comp arrow
  % Midpoint of the arrow between affil and cons
  \path let \p1 = ($(cons)!0.5!(comp)$) in coordinate (midAffCons) at (\x1,\y1);

  % The new box above that midpoint
  \node[note, above=1.4cm of midAffCons] (lit) {efficiency $\uparrow$\\price $\uparrow$\\ quality ?};

  % Arrow from the middle of the affil<->cons arrow up to the box
  \draw[-, line width=1.2pt, draw=eggplant] (midAffCons) -- (lit.south);
...
% \textit{Effects are mitigated with more distance between the hospitals \small  \textcolor{lightgray}{(\cite{cooper2019price})} \large}
...
\begin{frame}{Less Regulated Consolidation}
    Don't often cause national attention, but could be anti-competitive:
    ...
    \begin{columns}[T]
        ...
        \item ``Stealth mergers"
            \item Shown to reduce competition \\ \small \textcolor{lightgray}{(\cite{majerovitz2021consolidation}; \cite{aggarwal2025stealth}; \cite{wollmann2019stealth}; \cite{wollmann2020get}; \cite{kepler2023stealth})} \large
        \end{wideitemize}
        ...
        \begin{wideitemize}
            \item Joint ventures; Clinical agreements; \textit{Shared members of leadership}
            \item Recent increased state regulation
        \end{wideitemize}
    \end{columns}
\end{frame}

\begin{frame}{Informal Affiliations: Overlapping Leadership}
    In this paper, I specifically measure \underline{overlapping leadership}

    \vspace{2mm}
    \begin{wideitemize}
        \item A board member on the board of two competing hospitals OR
        \item An executive who sits on the board of a competing hospital
    \end{wideitemize}
    ...
    \vspace{2mm}
    \begin{wideitemize}
        \item Illegal according to the Clayton Act
...
\begin{itemize}
    \item Tax Forms 990s: sample of 1,200 nonprofit hospitals
    \item 2013-2023
\end{itemize}
...
\animategraphics[
  ...
  loop,
  width=0.8\linewidth
]{1}{Objects/frames_hrr_conn_gif/frame_}{01}{08}
%   ^fps       ^prefix         ^first ^last
\end{frame}
...
%     \begin{figure}
%         \centering
%         \includegraphics[width=0.75\linewidth]{Objects/system.pdf}
%     \end{figure}
% \end{frame}

\begin{frame}{Testing Using the ``Cost" of Merging}
    Leveraging how ``aggressive" national merger regulation as a shock to informal affiliation
...
\begin{frame}{What if Mergers Become More/Less Costly?}
    ...
    $$\text{Overlap}_{ht} = MergerCost_t + \alpha_h $$
    $$\text{Overlap}_{ht} = MergerCost_{t-1} + \alpha_h $$
\end{frame}
...
\textcolor{purple}{$\implies$ No relationship when considering overlap in the same market\\
\vspace{2mm}
$\implies$ Substitutes when choice set includes out of market}
\end{frame}
...
\section{Leadership Overlap and Hospital Behavior}

\begin{transitionframe}
    \LARGE \centering
    Leadership Overlap and Hospital Behavior
\end{transitionframe}
...
\begin{frame}{Estimation}
    Much like merging, informal affiliation is \underline{not random}

    \vspace{3mm}
    \begin{wideitemize}
        \item Rely on more valid comparison groups
        \item Treatment: gaining leadership overlap in the same HRR
        \item Only compare to not-yet-treated hospitals
    \end{wideitemize}
\end{frame}

\begin{frame}{Estimation: Stacked DiD}
    Staggered treatment: gaining overlap in the \underline{same market}:
    ...
    \begin{itemize}
        ...
        \item No other events correlated with gaining same-HRR overlap
    \end{itemize}
\end{frame}
...
\begin{frame}{Overlap and Quality}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth]{Objects/overall_rating_gainonly.pdf}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth]{Objects/summary_rating_gainonly.pdf}
...
\begin{frame}{Overlap and Quality}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth]{Objects/care_transition_gainonly.pdf}
...
\begin{columns}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth]{Objects/any_purch_gainonly.pdf}
        \caption{Any Purchases}
    \end{figure}

    \column{0.5\linewidth}
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth]{Objects/tot_operating_exp_gainonly.pdf}
...
\begin{columns}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth]{Objects/tot_discharges_gainonly.pdf}
...
\begin{columns}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth]{Objects/perc_mcare_gainonly.pdf}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth]{Objects/perc_mcaid_gainonly.pdf}
...
\begin{columns}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth]{Objects/bed_conc_gainonly.pdf}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth]{Objects/num_services_gainonly.pdf}
...
\begin{frame}{Conclusion}
    \begin{block}{}
        Should we devote resources to regulating informal affiliations as concern over lack of competition rises?
    \end{block}

    \vspace{8mm}

    \begin{wideitemize}
        \item Within a market, formal and informal affiliations are unrelated
        \begin{itemize}
            \item But outside of the market, they are substitutes
        \end{itemize}
        ...
        \begin{itemize}
            ...
            \item No change in patient flows (in aggregate) or investment
        \end{itemize}
